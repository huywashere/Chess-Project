...
async function main() {
  ...
  for (const u of usersData) {
    ...
    console.log(
      `✅ User: ${user.title ? `[${user.title}] ` : ""}${user.username} (ELO: ${user.eloRating})`
    );
  }
  ...
  const tournamentsData = [
    {
      ...
      description:
        "Giải đấu chớp cuồng nhiệt hàng ngày. Hệ thống ghép cặp liên tục, tích lũy chuỗi thắng lửa để bứt phá bảng vàng.",
      format: "Arena",
      timeControl: "3+0",
      status: "LIVE",
      maxPlayers: 256,
    ...
    },
    {
      ...
      description:
        "Giải đấu đỉnh cao 9 vòng hệ Thụy Sĩ quy tụ các Đại Kiện Tướng hàng đầu Việt Nam và Đông Nam Á.",
      format: "Swiss",
      timeControl: "10+5",
      status: "UPCOMING",
      maxPlayers: 128,
  ...
  if (createdUsers.length >= 2) {
    ...
    if (!existingGame) {
      ...
      console.log(
        `♟️ Sample Game & Deep Analysis created: ${white.username} vs ${black.username}`
      );
    }
  }
